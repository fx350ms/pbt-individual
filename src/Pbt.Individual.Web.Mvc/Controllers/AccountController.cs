...
using Pbt.Individual.Web.Models.Account;
using Pbt.Individual.Web.Views.Shared.Components.TenantChange;
...
public class AccountController : IndividualControllerBase
{
    ...
    private async Task<ActionResult> RegisterForExternalLogin(ExternalLoginInfo externalLoginInfo)
    {
        ...
        var nameinfo = ExternalLoginInfoHelper.GetNameAndphoneNumberFromClaims(externalLoginInfo.Principal.Claims.ToList());

        var viewModel = new RegisterViewModel
        {
            EmailAddress = email,
            Name = nameinfo.name,
            PhoneNumber = nameinfo.phoneNumber,
            IsExternalLogin = true,
            ExternalLoginAuthSchema = externalLoginInfo.LoginProvider
        };

        if (nameinfo.name != null &&
            nameinfo.phoneNumber != null &&
            email != null)
        {
